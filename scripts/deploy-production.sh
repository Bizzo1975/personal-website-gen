...
# Production Deployment Script for Personal Website
# This script automates the deployment process to Digital Ocean + CloudFlare
...
LOG_FILE="/home/deploy/deployment.log"
...
setup_monitoring() {
    ...
    cat > /home/deploy/backup-production.sh << 'EOF'
#!/bin/bash
BACKUP_DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_DIR="/home/deploy/backups"
mkdir -p $BACKUP_DIR
...
EOF
    ...
    (crontab -l 2>/dev/null; echo "0 2 * * * /home/deploy/backup-production.sh >> /home/deploy/backup.log 2>&1") | crontab -
...
display_summary() {
    ...
    echo "Run backup: /home/deploy/backup-production.sh"
    echo ""
    echo "=== NEXT STEPS ==="
