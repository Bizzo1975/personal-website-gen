...
import { useAriaAnnouncements, ScreenReaderOnly } from '@/components/AccessibilityEnhancements';
...
const SocialShare: React.FC<SocialShareProps> = ({
  ...
  const handleShare = (platform: SocialPlatform) => {
    const shareUrl = platform.shareUrl({ url, title, description, hashtags, via });
    ...
    announce(`Sharing to ${platform.name}`);
  };
      ...
      <div className="flex items-center gap-2">
        {socialPlatforms.map((platform) => (
          <button
            key={platform.name}
            onClick={() => handleShare(platform)}
            className={cn(buttonBaseClasses, platform.color)}
            aria-label={`Share on ${platform.name}`}
            title={`Share on ${platform.name}`}
          >
            {platform.icon}
            <ScreenReaderOnly>Share on {platform.name}</ScreenReaderOnly>
          </button>
        ))}
        ...
        >
          ...
          <ScreenReaderOnly>
            {copiedToClipboard ? 'Link copied to clipboard' : 'Copy link to clipboard'}
          </ScreenReaderOnly>
        </button>
      </div>
    </div>
