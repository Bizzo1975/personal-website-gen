...
2. **Build and start all services using the reliable startup script:**
   ```bash
   # Make the script executable
   chmod +x scripts/start-production-reliable.sh
   
   # Run the comprehensive startup script
   ./scripts/start-production-reliable.sh
   ```
   
   **What this script does:**
   - Verifies all prerequisites (.env.production, Docker, etc.)
   - Loads environment variables correctly
   - Ensures database is running and healthy
   - Verifies database has data (profiles, posts, projects)
   - Builds app container if needed (with BuildKit disabled)
   - Starts app container with proper environment variables
   - Verifies app can connect to database
   - Shows final status of all services
   
   **This is the RECOMMENDED way to start production** - it handles all edge cases automatically.
   
   **Alternative: Manual build (if script not available):**
   ```bash
   ...
   # Build with BuildKit disabled (prevents buildx errors)
   DOCKER_BUILDKIT=0 docker-compose -f docker-compose.prod.yml build app
   
   # Start all services
   DOCKER_BUILDKIT=0 docker-compose -f docker-compose.prod.yml up -d
   ```
   
   **Troubleshooting: BuildKit Error**
   
   If you see: `BuildKit is enabled but buildx component is missing or broken`
   
   **Solution: Always use DOCKER_BUILDKIT=0 (the startup script does this automatically):**
   ```bash
   DOCKER_BUILDKIT=0 docker-compose -f docker-compose.prod.yml build app
   ```
   
   **Troubleshooting: zlib Error During Build**
   
   If you see: `npm error zlib: incorrect data check` in the development stage
   
   **This happens because Docker tries to build all stages. Solution:**
   ```bash
   # Clean Docker cache and rebuild
   docker system prune -a
   DOCKER_BUILDKIT=0 docker-compose -f docker-compose.prod.yml build --target runner app
   ```
   
   **Or use the reliable startup script which handles this automatically.**

3. **The startup script automatically starts all services. If you need to start manually:**
   ```bash
   ...
   # Start all services with BuildKit disabled
   DOCKER_BUILDKIT=0 docker-compose -f docker-compose.prod.yml up -d
   ```
   ...
   **Breaking this down:**
   ...
   - `DOCKER_BUILDKIT=0`: Prevents buildx errors
   ...
   **IMPORTANT:** Always load environment variables before running docker-compose commands, or use the startup script which does this automatically.
...
4. Cloudflare Tunnel to reconnect automatically (configured in Step 8)

**📋 Correct Order of Operations:**
1. ✅ **Step 7.6.1** - Configure Proxmox VM Auto-Start (do this now)
2. ✅ **Step 7.6.2** - Verify Docker Service Auto-Start (do this now)
3. ✅ **Step 7.6.3** - Configure Docker Compose Auto-Start (do this now)
4. ⏭️ **Step 7.6.4** - Verify Cloudflare Tunnel Auto-Start (SKIP - do after Step 8)
5. ⏭️ **Step 7.6.5** - Test Auto-Startup (SKIP - do after Step 7A and Step 8)
6. ✅ **Step 7.7** - Verify Application is Running (do this now)
7. ✅ **Step 7A** - Restore Database (do this next)
8. ✅ **Step 8** - Configure Cloudflare Tunnel (do this after database restore)
9. ✅ **Return to Step 7.6.4** - Verify Cloudflare Tunnel Auto-Start (after Step 8)
10. ✅ **Return to Step 7.6.5** - Test Auto-Startup (after Step 8)
...
1. **First, create the wrapper script that loads environment variables:**
   ```bash
   nano /opt/app/site/scripts/start-docker-compose.sh
   ```
   ...
   **Make it executable:**
   ```bash
   chmod +x /opt/app/site/scripts/start-docker-compose.sh
   ```
...
3. **Add the following content:**
   ...
   ExecStart=/opt/app/site/scripts/start-docker-compose.sh up -d
   ExecStop=/opt/app/site/scripts/start-docker-compose.sh down
   TimeoutStartSec=300
   User=prod-admin
   Group=docker
   ...
   **Breaking this down:**
   ...
   - `ExecStart`: Uses wrapper script that loads environment variables, then starts containers
   - `ExecStop`: Uses wrapper script to stop containers gracefully
   - `User=prod-admin`: Run as your production admin user (replace with your actual username if different)
   - `Group=docker`: User must be in docker group
   - `TimeoutStartSec=300`: Allows 5 minutes for containers to start (builds can take time)
   - `RemainAfterExit=yes`: Service stays active even after containers start
   
   **IMPORTANT:** Replace `prod-admin` with your actual username if different. To find your username:
   ```bash
   whoami
   ```
...
8. **Start the service now (optional - to test):**
   ```bash
...
10. **Check service status:**
   ...
   **If you see errors:**
   - Check the username is correct: `whoami` (should match `User=` in service file)
   - Verify user is in docker group: `groups` (should include `docker`)
   - Check wrapper script is executable: `ls -l /opt/app/site/scripts/start-docker-compose.sh`
   - View detailed logs: `sudo journalctl -xeu docker-compose-app.service -n 50`

#### Step 7.6.4: Verify Cloudflare Tunnel Auto-Start

**⚠️ SKIP THIS STEP FOR NOW - Do this AFTER Step 8 (after Cloudflare Tunnel is installed)**

**Why skip now:** Cloudflare Tunnel (cloudflared) hasn't been installed yet. It will be installed in Step 8. Come back to this step after completing Step 8.

**When to do this:** After you've completed Step 8 (Configure Cloudflare Tunnel), return here to verify auto-start is configured.

**On the production VM (after Step 8 is complete):**
...
**⚠️ SKIP THIS STEP FOR NOW - Do this AFTER Step 8 and Step 7A (after Cloudflare Tunnel is installed and database is restored)**

**Why skip now:** 
- Cloudflare Tunnel hasn't been installed yet (Step 8)
- Database hasn't been restored yet (Step 7A)
- Website isn't publicly accessible yet

**When to do this:** After you've completed:
1. Step 7A (Restore Database)
2. Step 8 (Configure Cloudflare Tunnel)
3. Then return here to test auto-startup

**To verify everything works after a reboot (after Steps 7A and 8 are complete):**
...
3. **SSH back into the VM and verify:**
   ...
   # Check Cloudflare Tunnel is running (only after Step 8)
   sudo systemctl status cloudflared
...
- If Cloudflare Tunnel doesn't start (after Step 8), check: `sudo journalctl -u cloudflared -n 50`
...
**CRITICAL:** After restoring the database, you MUST run the startup script (Step 7A.5) to ensure the app connects to the database properly. Simply restoring the database is not enough - the app container must be started with proper environment variables.
...
1. **Locate your database backup:**
   - Your backups may be in various formats:
     - ZIP file: `FULL_BACKUP_LINUX_YYYYMMDD_HHMMSS.zip` (contains database and other files)
     - Compressed SQL: `database_backup_YYYYMMDD_HHMMSS.sql.gz` or `backup_YYYYMMDD_HHMMSS.sql.gz`
     - Plain SQL: `database_backup_YYYYMMDD_HHMMSS.sql`
   - Note the full path to this file
   - **For this guide, we'll use:** `C:\Users\jonkd\OneDrive\Documents\FULL_BACKUP_LINUX_20251109_181515.zip`

2. **Verify the backup file exists:**
   ```bash
   # On Windows (Command Prompt or PowerShell):
   dir "C:\Users\jonkd\OneDrive\Documents\FULL_BACKUP_LINUX_*.zip"
   
   # Or check if it's a SQL file:
   dir "C:\Users\jonkd\OneDrive\Documents\*.sql*"
   ```
   
   **What to look for:** Should see your backup file with a file size (not 0 bytes).
...
- **Use Tailscale IP** if Tailscale is installed on both your Windows machine and the production VM (recommended - works from anywhere, more secure)
- **Use Production VM IP (192.168.20.20)** only if you're on the same local network and have direct access

**To find your production VM's Tailscale IP:**
1. Open Tailscale on your Windows machine
2. Look for your production VM in the device list
3. Note its Tailscale IP (usually starts with `100.x.x.x`)

1. **From your Windows machine, transfer the ZIP backup file:**

   **If using PowerShell or WSL (Windows Subsystem for Linux):**
   ```powershell
   # If using Tailscale to access the production server (RECOMMENDED):
   scp "C:\Users\jonkd\OneDrive\Documents\FULL_BACKUP_LINUX_20251109_181515.zip" prod-admin@<prod-server-tailscale-ip>:/home/prod-admin/
   
   # Or if using the DMZ IP (only if on same local network):
   scp "C:\Users\jonkd\OneDrive\Documents\FULL_BACKUP_LINUX_20251109_181515.zip" prod-admin@192.168.20.20:/home/prod-admin/
   ```
   ...
   **Note:** The file path shown is for the ZIP backup file. If your backup is in a different location or format, adjust the path accordingly.

   **Alternative: Using WinSCP (GUI tool for Windows):**
   1. Download WinSCP from https://winscp.net/
   2. Connect using:
      - **Host name:** Your Tailscale IP (e.g., `100.x.x.x`) OR `192.168.20.20`
      - **User name:** `prod-admin`
      - **Password:** Your production VM password
   3. Navigate to `/home/prod-admin/` on the server
   4. Drag and drop your backup file from Windows Explorer
   
   **What this does:** Copies the file from your Windows computer to the server's home directory.
   
   **You'll be prompted for:**
   - The server password (the one you set for `prod-admin` during Ubuntu installation)
...
1. **Navigate to your home directory and verify the backup file:**
   ```bash
   cd ~
   ls -lh FULL_BACKUP_LINUX_*.zip
   ```
   
   **What this does:** Lists the backup ZIP file to confirm it's there and check its size.
   
   **What to look for:** Should see `FULL_BACKUP_LINUX_20251109_181515.zip` (or similar) with a file size.
...
2. **Install unzip if not already installed:**
   ...
   **What this does:** Installs the `unzip` tool needed to extract ZIP files.

3. **Extract the ZIP backup file:**
   ```bash
   unzip FULL_BACKUP_LINUX_20251109_181515.zip
   ```
   
   **What this does:** Extracts all files from the ZIP archive into the current directory.
   
   **What you'll see:** A list of files being extracted. Look for files like:
   - `database_backup_*.sql` or `backup_*.sql` (the database backup)
   - Other files (uploads, images, etc.)
   
   **Note:** The exact filename may vary. Common names include:
   - `database_backup_YYYYMMDD_HHMMSS.sql`
   - `backup_YYYYMMDD_HHMMSS.sql`
   - `personal_website_backup.sql`

4. **Find the database SQL file:**
   ```bash
   ls -lh *.sql
   ```
   
   **What this does:** Lists all `.sql` files in your home directory.
   
   **What to look for:** Should see one or more SQL files. Note the exact filename - you'll need it in the next step.
   
   **If you see multiple SQL files:** Look for the largest one (usually the main database backup) or check the filename for clues like "database" or "backup".

5. **Ensure the database exists (create if needed):**
   ```bash
   cd /opt/app/site
   docker-compose -f docker-compose.prod.yml exec db psql -U postgres -c "CREATE DATABASE personal_website;" 2>/dev/null || echo "Database already exists (this is OK)"
   ```
   
   **What this does:** Creates the database if it doesn't exist. If it already exists, you'll see a message (which is fine).
   
   **Note:** If you need to start fresh (database already has data you want to replace), you can drop and recreate it:
   ```bash
   # WARNING: This deletes all existing data in the database!
   # Only run this if you want to completely replace the database
   docker-compose -f docker-compose.prod.yml exec db psql -U postgres -c "DROP DATABASE IF EXISTS personal_website;"
   docker-compose -f docker-compose.prod.yml exec db psql -U postgres -c "CREATE DATABASE personal_website;"
   ```

6. **Check and fix file encoding (if needed):**
   
   **If you get an encoding error like "invalid byte sequence for encoding UTF8: 0xff":**
   
   ```bash
   cd ~
   # Check file encoding
   file database/database_backup_complete.sql
   
   # Check for BOM (Byte Order Mark) - first few bytes
   head -c 3 database/database_backup_complete.sql | od -An -tx1
   ```
   ...
   **If the file has encoding issues, convert it to UTF-8:**
   ...
   # Convert line endings and remove BOM
   dos2unix database/database_backup_complete.sql
   
   # Or use sed to remove BOM if present
   sed -i '1s/^\xEF\xBB\xBF//' database/database_backup_complete.sql
   
   # If file is UTF-16, convert to UTF-8 (install iconv if needed)
   # sudo apt install -y iconv
   # iconv -f UTF-16LE -t UTF-8 database/database_backup_complete.sql > database/database_backup_complete_utf8.sql
   # mv database/database_backup_complete_utf8.sql database/database_backup_complete.sql
   ```

7. **Import the database backup:**
   ```bash
   cd ~
   # Use the correct path: database/database_backup_complete.sql
   docker-compose -f /opt/app/site/docker-compose.prod.yml exec -T db psql -U postgres -d personal_website < database/database_backup_complete.sql
   ```
   
   **Breaking this down:**
   - `cd ~`: Go back to home directory where the SQL file is
   ...
   - `< database/database_backup_complete.sql`: Feed the SQL file into the database (note the `database/` subdirectory)
   ...
   **Alternative method (if direct import fails):**
   ```bash
   # Copy file into container first, then import
   docker cp ~/database/database_backup_complete.sql $(docker-compose -f /opt/app/site/docker-compose.prod.yml ps -q db):/tmp/backup.sql
   docker-compose -f /opt/app/site/docker-compose.prod.yml exec db psql -U postgres -d personal_website -f /tmp/backup.sql
   docker-compose -f /opt/app/site/docker-compose.prod.yml exec db rm /tmp/backup.sql
   ```

8. **Verify the import was successful:**
   ```bash
   # Check that tables exist
   docker-compose -f /opt/app/site/docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "\dt"
   
   # Check critical tables have data
   docker-compose -f /opt/app/site/docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "SELECT COUNT(*) FROM profiles;"
   ...
   docker-compose -f /opt/app/site/docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "SELECT COUNT(*) FROM pages;"
   ```
   
   **What this does:** 
   - `\dt` lists all tables in the database
   - The COUNT queries verify each critical table has data
   - You should see numbers matching your development database
   
   **Critical check:** The `profiles` table MUST have at least 1 row, or the website will show "Profile data not found" errors.
   ...
   **If you see errors:** Check that:
   ...
   - All required tables exist (profiles, posts, projects, pages, etc.)

9. **Clean up the backup files (optional, for security):**
   ```bash
   cd ~
   # Remove extracted SQL file
   rm *.sql
   
   # Optionally remove the ZIP file (keep your original on Windows!)
   # rm FULL_BACKUP_LINUX_*.zip
   ```
   
   **Why:** The backup files contain sensitive data. Once imported, you can delete them from the server (keep your original backup safe on your development machine!)
   
   **Note:** It's recommended to keep the ZIP file on the server temporarily until you've verified everything works, then you can delete it.

### Step 7A.5: Start Production Website with Restored Database

**CRITICAL STEP:** After restoring the database, you MUST start the website properly. Simply having the database restored is not enough - the app container must be started with proper environment variables to connect to the database.

**Use the reliable startup script (RECOMMENDED - This is your single source of truth):**
```bash
cd /opt/app/site

# Transfer the script from Windows if not already there:
# On Windows: scp scripts/start-production-reliable.sh prod-admin@192.168.20.20:/opt/app/site/scripts/
...
**This script will:**
- ✅ Verify .env.production exists and load environment variables
- ✅ Create DATABASE_URL if missing (from POSTGRES_PASSWORD)
- ✅ Verify database container is running and healthy
- ✅ Verify database has data (profiles, posts, projects)
- ✅ Build app container if needed (skips development stage to avoid zlib errors)
- ✅ Start app container with proper environment variables
- ✅ Verify DATABASE_URL is set in app container
- ✅ Test database connection from app container
- ✅ Show final status of all services

**This script handles ALL edge cases automatically. Use it.**

**If the script is not available, start manually:**
```bash
cd /opt/app/site
...
if [ -z "$DATABASE_URL" ] && [ -n "$POSTGRES_PASSWORD" ]; then
    echo "DATABASE_URL=postgresql://postgres:${POSTGRES_PASSWORD}@db:5432/personal_website" >> .env.production
    export DATABASE_URL="postgresql://postgres:${POSTGRES_PASSWORD}@db:5432/personal_website"
fi
...
docker-compose -f docker-compose.prod.yml exec app node -e "const {Pool}=require('pg');const p=new Pool({connectionString:process.env.DATABASE_URL});p.query('SELECT COUNT(*) FROM profiles').then((r)=>console.log('SUCCESS: ' + r.rows[0].count + ' profiles')).catch((e)=>console.log('FAILED: ' + e.message));"
```

### Step 7A.6: Verify Database Content in Application

**IMPORTANT:** Before verifying in the application, run comprehensive database diagnostics to ensure data was imported correctly.

1. **Transfer and run the diagnostic script:**
   
   **From Windows, transfer the diagnostic script:**
   ```powershell
   scp "F:\Github\personal-website-gen\scripts\verify-database-restore.sh" prod-admin@<prod-ip>:/opt/app/site/scripts/
   ```
   
   **On production server:**
   ```bash
   cd /opt/app/site
   chmod +x scripts/verify-database-restore.sh
   ...
   **What this checks:**
   ...
   - All tables exist
   - Data counts in critical tables (profiles, posts, projects, users, pages)
   - App container can connect to database
   
   **Critical:** The `profiles` table MUST have at least 1 record, or the app will show "Profile data not found" errors.

2. **Quick manual verification (if script not available):**
   ```bash
   cd /opt/app/site
   
   # Check profiles table (CRITICAL - must have data)
   docker-compose -f docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "SELECT COUNT(*) FROM profiles;"
   
   # Check posts table
   docker-compose -f docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "SELECT COUNT(*) FROM posts;"
   
   # Check projects table
   docker-compose -f docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "SELECT COUNT(*) FROM projects;"
   
   # List all tables
   docker-compose -f docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "\dt"
   
   # Test app container database connection
   docker-compose -f docker-compose.prod.yml exec app printenv DATABASE_URL
   ```

3. **The startup script (Step 7A.5) already restarts the app. If you need to restart manually:**
   ```bash
   cd /opt/app/site
...
4. **Check application logs:**
   ...
   **What to look for:**
   - Should see "PostgreSQL connection successful" messages
   - No database connection errors
   - No "Profile data not found" errors (if profiles table has data)
...
- **"Profile data not found" error in application:**
  
  **This error means the `profiles` table is empty or doesn't exist.**
  
  ```bash
  # Step 1: Check if profiles table exists and has data
  docker-compose -f /opt/app/site/docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "SELECT COUNT(*) FROM profiles;"
  
  # Step 2: If count is 0, check what tables were imported
  docker-compose -f /opt/app/site/docker-compose.prod.yml exec db psql -U postgres -d personal_website -c "\dt"
  
  # Step 3: Check if backup file contains profiles table
  head -200 ~/database/database_backup_complete.sql | grep -i "CREATE TABLE.*profiles"
  
  # Step 4: If profiles table exists but is empty, check if import actually ran
  # Look for INSERT statements in backup
  grep -i "INSERT INTO profiles" ~/database/database_backup_complete.sql | head -5
  ```
  
  **If profiles table is empty but backup has data:**
  - The database import may have failed partway through
  - Try re-importing the database (drop and recreate first if needed)
  - Check database logs for errors during import
...
- **If you see "out of memory" errors, the database might be too large - check available RAM:**
  ```bash
  free -h
  ```
...
- **If tables are empty after import, check the backup file was complete and not corrupted:**
  ...
  ls -lh ~/database/database_backup_complete.sql
  
  # Check if backup contains table creation statements
  head -100 ~/database/database_backup_complete.sql | grep -i "CREATE TABLE"
  ```
...
- **If database connection fails in application, verify DATABASE_URL:**
  
  **The most common issue is DATABASE_URL not being set in the app container.**
  
  ```bash
  # Step 1: Check if DATABASE_URL exists in .env.production
  cd /opt/app/site
  grep "^DATABASE_URL=" .env.production
  
  # Should show: DATABASE_URL=postgresql://postgres:YOUR_PASSWORD@db:5432/personal_website
  # If empty, you need to add it!
  
  # Step 2: Check if app container can see DATABASE_URL
  docker-compose -f docker-compose.prod.yml exec app printenv DATABASE_URL
  
  # If this returns nothing, DATABASE_URL is not being passed to the container
  
  # Step 3: Verify password in DATABASE_URL matches POSTGRES_PASSWORD
  grep "^POSTGRES_PASSWORD=" .env.production
  # Extract password from DATABASE_URL and compare
  ```
  
  **If DATABASE_URL is missing or incorrect:**
  
  1. **Edit .env.production and add/update DATABASE_URL:**
     ```bash
     cd /opt/app/site
     nano .env.production
     ```
     
     **Add or update this line (replace YOUR_PASSWORD with actual POSTGRES_PASSWORD):**
     ```
     DATABASE_URL=postgresql://postgres:YOUR_PASSWORD@db:5432/personal_website
     ```
     
     **Important:** The password in DATABASE_URL MUST match POSTGRES_PASSWORD exactly.
  
  2. **Restart app container to pick up the change:**
     ```bash
     docker-compose -f docker-compose.prod.yml restart app
     sleep 10
     ```
  
  3. **Verify DATABASE_URL is now visible:**
     ```bash
     docker-compose -f docker-compose.prod.yml exec app printenv DATABASE_URL
     ```
  
  4. **Test database connection from app container:**
     ```bash
     docker-compose -f docker-compose.prod.yml exec app node -e "const {Pool}=require('pg');const p=new Pool({connectionString:process.env.DATABASE_URL});p.query('SELECT NOW()').then(()=>console.log('SUCCESS')).catch(e=>console.log('FAILED:',e.message));"
     ```
     
     **Should output:** `SUCCESS`
  
  **Quick fix script (if DATABASE_URL is missing):**
  ```bash
  cd /opt/app/site
  ...
  if ! grep -q "^DATABASE_URL=" .env.production; then
      ...
      sed -i "s|^DATABASE_URL=.*|DATABASE_URL=postgresql://postgres:${POSTGRES_PASSWORD}@db:5432/personal_website|" .env.production
      echo "Updated DATABASE_URL in .env.production"
  fi
...
1. **Add Cloudflare's GPG key and repository:**
```bash
   # Download and add Cloudflare's GPG key
   curl -L https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64.deb -o /tmp/cloudflared.deb
   sudo dpkg -i /tmp/cloudflared.deb
   ```
   
   **What this does:** Downloads the official Cloudflared package directly from Cloudflare and installs it.
   ...
   **What this does:** Alternative method using wget, and fixes any missing dependencies.
...
2. **Verify installation:**
   ...
   **What to look for:** Should display version information (e.g., `cloudflared version 2024.x.x`).
   
   **If you see an error:** Make sure you're running as root or with sudo for the installation step.
